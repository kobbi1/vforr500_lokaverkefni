...
<form  action="{{ $timer->path() }}" method="POST" class="w-full max-w-md">
	...
	<div class="flex flex-wrap -mx-3 mb-6 customCard">
    	<div class="form-group">
    		...
    		<input class="form-control text-center" id="grid" placeholder="Pizza" type="text"  name="title" value="{{$timer->title}}">
    	</div>
    	<div class="form-group">
    		<h3 class="text-center">Time</h3>
            <input type="number" class="form-control text-center" placeholder="Minutes" min="1" max="5999" id="grid" name="timer" value="{{$timer->timer}}">
    	</div>
